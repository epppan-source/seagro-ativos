...
import { QrCode, Search, Camera, CheckCircle, XCircle, User, Archive, Wrench } from "lucide-react"

type Destino = "FUNCIONARIO" | "DEPOSITO" | "MANUTENCAO"
...
const DESTINO_BADGE: Record<Exclude<Destino, "FUNCIONARIO">, { label: string; classe: string; Icone: typeof Archive }> = {
  DEPOSITO: { label: "Depósito", classe: "bg-emerald-100 text-emerald-700", Icone: Archive },
  MANUTENCAO: { label: "Manutenção", classe: "bg-orange-100 text-orange-700", Icone: Wrench },
}
...
export default function TransferenciasPage() {
  ...
  const [destino, setDestino] = useState<Destino>("FUNCIONARIO")
  ...
  async function solicitar(e: React.FormEvent) {
    ...
    if (destino === "FUNCIONARIO" && !novoResponsavelId) {
      setErro("Selecione o novo responsavel.")
      return
    }
    setSalvando(true)
    try {
      await api.post("/api/transferencias", {
        ativo_id: ativoId,
        destino,
        novo_responsavel_id: destino === "FUNCIONARIO" ? novoResponsavelId : null,
  ...
  return (
    <div>
      ...
      {mostrarForm && (
        <div className="bg-white rounded-xl shadow p-5 mb-6">
          ...
          <form onSubmit={solicitar} className="space-y-4">
            ...
            {/* Destino (comum aos dois modos): pra outra pessoa, pro deposito ou pra manutencao */}
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-2">Destino</label>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => setDestino("FUNCIONARIO")}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg text-sm font-medium border-2 min-h-[44px] transition-colors ${
                    destino === "FUNCIONARIO"
                      ? "border-green-700 bg-green-50 text-green-700"
                      : "border-gray-200 text-gray-500 hover:border-gray-300"
                ...
                <button
                  type="button"
                  onClick={() => { setDestino("DEPOSITO"); setNovoResponsavelId("") }}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg text-sm font-medium border-2 min-h-[44px] transition-colors ${
                    destino === "DEPOSITO"
                ...
                >
                  <Archive size={16} /> Depósito
                </button>
                <button
                  type="button"
                  onClick={() => { setDestino("MANUTENCAO"); setNovoResponsavelId("") }}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg text-sm font-medium border-2 min-h-[44px] transition-colors ${
                    destino === "MANUTENCAO"
            ...
            {/* Novo responsavel: só aparece quando o destino é "Pessoa" */}
            {destino === "FUNCIONARIO" && (
              <div>
            ...
            <button
              disabled={salvando || !ativoId || (destino === "FUNCIONARIO" && !novoResponsavelId)}
              type="submit"
      ...
      <div className="space-y-3">
        {lista.map((t) => {
          const badge = t.destino !== "FUNCIONARIO" ? DESTINO_BADGE[t.destino] : null
          return (
            <div key={t.id} className="bg-white rounded-lg shadow p-4">
              <div className="mb-3">
                ...
                <p className="text-sm text-gray-500 mt-1 flex flex-wrap items-center gap-1.5">
                  ...
                  <span>→</span>
                  {badge ? (
                    <span className={`inline-flex items-center gap-1 font-medium px-2 py-0.5 rounded-full text-xs ${badge.classe}`}>
                      <badge.Icone size={12} /> {badge.label}
                    </span>
                  ) : (
                    <span className="font-medium">{t.novo_responsavel_id ? nomeFuncionario(t.novo_responsavel_id) : "—"}</span>
                  )}
                </p>
